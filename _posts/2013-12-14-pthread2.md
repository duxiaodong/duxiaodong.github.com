...
##互斥我吧！
在[前一篇文章](http://junevimer.github.io/2013/12/14/pthread1.html)中 ，谈到了会导致异常结果的线程代码。两个线程分别对同一个全局变量进行了二十次加一。变量的值最后应该是 40，但最终值却是 21。这是怎么回事呢？因为一个线程不停地“取消”了另一个线程执行的加一操作，所以产生这个问题。现在让我们来查看改正后的代码，它使用 互斥对象(mutex)来解决该问题：
...
![image](https://github.com/junevimer/Picture/raw/master/pthread2.gif)
