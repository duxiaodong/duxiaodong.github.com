...
##条件变量详解
在[前一篇文章](http://junevimer.github.io/2013/12/14/pthread2.html)结束时，我描述了一个比较特殊的难题：如果线程正在等待某个特定条件发生，它应该如何处理这种情况？它可以重复对互斥对象锁定和解锁，每次都会检查共享数据结构，以查找某个值。但这是在浪费时间和资源，而且这种繁忙查询的效率非常低。解决这个问题的最佳方法是使用 pthread\_cond\_wait() 调用来等待特殊条件发生。
...
![image](https://github.com/junevimer/Picture/raw/master/pthread3.gif)
